...
import { generateWeatherWidget } from "./generate-weather-widget";
...
async function generateDaily(cityOne, cityTwo) {
  ...
  const conditions = await generateWeatherWidget(
    '',
    "forecast",
    weatherReportArray[0].currentConditionsIcon
  );

  const feelsLike = await generateWeatherWidget(
  ...
  const humidity = await generateWeatherWidget(
  ...
  const temperature = await generateWeatherWidget(
